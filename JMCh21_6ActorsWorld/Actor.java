...
public class Actor implements Comparable<Actor>
{
    ...
    private ArrayList<String> announced = new ArrayList<String>();
    ...
    public void readMail()
    {
        ...
        while (!mailbox.isEmpty() && !allSetFlag) {
            Message msg = mailbox.remove();
            switch (msg.getText().substring(0, 4)) {
                case "need":
                    if (countPossessions(msg.getText().substring(5)) > 1) {
                        reply(msg.getSender(), "have " + msg.getText().substring(5));
                    }
                    break;
                case "have":
                    if (countPossessions(msg.getText().substring(5)) == 0) {
                        reply(msg.getSender(), "ship " + msg.getText().substring(5));
                    }
                    break;
                case "ship":
                    if (countPossessions(msg.getText().substring(5)) > 1) {
                        if (msg.getSender().receiveItem(this, msg.getText().substring(5))) {
                            myPossessions.remove(msg.getText().substring(5));
                        }
                    }
                    break;
            }
