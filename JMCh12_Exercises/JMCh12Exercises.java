...
public class JMCh12Exercises
{
    ...
    {
        return arrayOf3Ints()[0] + arrayOf3Ints()[1] + arrayOf3Ints()[2];  // TODO fix this!!
    }
    ...
    {
        // a) it has only 100 elements
        // b) true 
        // c) true 
        // d) false; it's used like a field
    ...
    {
        char[] rpsarray = {'r', 'r', 'r', 'p', 'p', 'p', 'p', 'p', 's', 's', 's', 's', 's', 's'};
        
        return rpsarray[((int)(14*(Math.random())))];  // TODO fix this!!
    }
    ...
    public String whatDoesMysteryCountCount()
    {
        ...
        String answer = mCount + "index of first non-zero integer"; 
    ...
    public static int[] makePythagoreanTriple( int m, int n )
    {
        ...
        return new int[] {Math.abs(m*m - n*n), 2*m*n, m*m + n*n};  
    }
    ...
    {
        int[] aray = new int[2*n-1];
        for (int i = 0; i < n; i++) {
            aray[i] = i+1;
        }
        
        for (int i = 0; i < n-1; i++) {
            aray[aray.length - i - 1] = i+1;
        }
    ...
    public static int computeScore( String word )
    {
        ...
        for (int i = 0; i < word.length(); i++) {
            total += letterScores[alphabet.indexOf(word.charAt(i))];
        }      
        return total;  
    }
    ...
    public boolean[] prob11abcdeTrueFalse()
    {
        ...
        // to question (a) - (e)
        return new boolean[]{true, true, false, false, true};  // TODO fix this
    }
    ...
    public void removeMin( ArrayList<Integer> list )
    {
        if (list.size() > 0) {
            ...
            for (int i = 0; i < list.size(); i++) {
                min = (list.get(i).compareTo(min) < 0) ? list.get(i) : min;
            }
    ...
    {
        Arrays.sort(sample);
        return sample[(sample.length/2)] == m;  
    }
    ...
    public static double averageTopTwo( int[] scores )
    {
        ...
        if ( scores[iMax2] > scores[iMax1] )
        {
            ...
            iMax1 = iMax2;
            iMax2 = temp;
            // System.out.println(iMax1 + " " + iMax2); 
        }
    ...
    public static double positiveMax( double[][] m )
    {
        ...
        for (int i = 0; i < m.length; i++) {
            for (int j = 0; j < m[i].length; j++) {
                mMax = (m[i][j] > 0 && m[i][j] > mMax) ? m[i][j] : mMax;
            }
        }
    ...
    public static boolean covers( double[][] m1, double[][] m2 )
    {
        ...
        return (greaterCount/totalCount) >= 0.5;   // TODO fix this!!
    }
